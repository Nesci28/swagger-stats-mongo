...
module.exports.extractApiOpParameters = function (swaggerSpec, pathDef, opDef) {
  ...
  if ("parameters" in opDef && opDef.parameters instanceof Array) {
    ...
    for (let k = 0; k < opParams.length; k++) {
...
module.exports.generateApiOpCallDef = function (
  swaggerSpec,
  pathDef,
  opDef,
  op,
  ...
  let callURI = fullPath;
  for (const pname in opParams) {
    const pDef = opParams[pname];
    if ("in" in pDef && pDef.in == "path") {
      let pVal = "abcde"; // string by default
      if ("type" in pDef) {
        switch (pDef.type) {
          case "integer":
          case "number":
...
module.exports.generateApiOpList = function (swaggerSpec) {
  const basePath = module.exports.getApiBasePath(swaggerSpec);
  debug("BasePath: %s", basePath);

  const apiOperationsList = [];

  for (const path in swaggerSpec.paths) {
    const pathDef = swaggerSpec.paths[path];

    // Create full path
    const fullPath = module.exports.getApiFullPath(basePath, path);
    ...
    for (let i = 0; i < operations.length; i++) {
      ...
      if (op in pathDef) {
        ...
        const opParams = module.exports.extractApiOpParameters(
          swaggerSpec,
          pathDef,
          opDef,
        );
        ...
        // Call Definition
        const opCallDef = module.exports.generateApiOpCallDef(
          swaggerSpec,
          pathDef,
          opDef,
          op,
        ...
        const label = `${opMethod} ${path}`;
        apiOperationsList.push({
          label,
          path: fullPath,
          method: opMethod,
          opCallDef,
...
for (const sc in module.exports.httpStatusCodes) {
  module.exports.httpStatuses.push(sc);
}

module.exports.getRandomHttpStatusCode = function () {
  const idx = module.exports.getRandomArbitrary(
    0,
    module.exports.httpStatusesRandom.length,
  );
  return module.exports.httpStatusesRandom[idx];
};

module.exports.getHttpStatusMessage = function (code) {
  return code in module.exports.httpStatusCodes
    ? module.exports.httpStatusCodes[code]
    : "UNKNOWN";
};
