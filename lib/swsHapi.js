...
class SwsHapi {
  ...
  async register(server, options) {
    // Init Mongo
    const swsMongo = new SwsMongo(options);
    await this.swsMongo.init();

    // Init Auth
    this.swsAuth = new SwsAuth(swsMongo);
    ...
    server.route({
      ...
      async handler(request, h) {
        const authResult = await this.swsAuth.processAuth(
          request.raw.req,
          request.raw.res,
        );
        if (!authResult) {
    ...
    server.route({
      ...
      async handler(request, h) {
        await this.swsAuth.processLogout(request.raw.req, request.raw.res);
        return h.abandon;
      },
      options: options.routeOptions,
    });
