...
class SwsAuth {
  ...
  async storeSessionID(sid) {
    ...
    const res = await this.swsMongo.insertSession({
      sid,
      tsSec,
    });
    console.log("res :>> ", res);
  ...
  async patchSessionId(sid, ms) {
    await this.swsMongo.patchBySidSession(sid, ms);
  }

  async removeSessionID(sid) {
    await this.swsMongo.archiveBySidSession(sid);
  }
  ...
  async expireSessionIDs() {
    ...
    for (const session of sessions) {
      if (session.tsSec < tsSec + 500) {
        archiveByIdSessionPromises.push(
          this.swsMongo.archiveByIdSessions(session._id),
        );
      }
    }
